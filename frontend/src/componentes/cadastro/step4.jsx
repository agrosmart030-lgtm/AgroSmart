...
export default function VerificationStep({ email, onVerificationSuccess, onResendCode }) {
  ...
  const handleVerify = async () => {
    ...
    try {
      // endpoint adjusted to match backend route
      const response = await axios.post('http://localhost:5001/api/registro/verify-code', {
        email,
        code
      });
  ...
  const handleResendCode = async () => {
    ...
    try {
      // Prefer calling the parent handler if provided, otherwise call backend directly
      if (typeof onResendCode === 'function') {
        await onResendCode();
      } else {
        // fallback: call backend send-code endpoint
        await axios.post('http://localhost:5001/api/registro/send-code', { email });
      }
      setCountdown(60);
      setCanResend(false);
      setVerificationCode(["", "", "", "", "", ""]);
